...
module.exports = (bot, availableCommands) => {
  ...
  availableCommands.push({ command: commandName, tags: ['all'] }); // Tambahkan commandName ke dalam daftar availableCommands dengan tags: tools

  bot.command(commandName, async (alice) => {
    const message = alice.message;
    ...
    if (message.reply_to_message && message.reply_to_message.photo) {
      ...
      try {
        const creatingMessage = await alice.reply('Creating...');
        ...
        await alice.replyWithSticker({ source: stickerPath });

        // Menghapus file stiker setelah dikirim
        await fs.unlink(stickerPath);

        // Edit pesan "Creating..." menjadi "Success!!!"
        await alice.telegram.editMessageText(alice.chat.id, creatingMessage.message_id, null, 'Success!!!');
      } catch (error) {
        console.error('Error generating sticker:', error);
        alice.reply('Terjadi kesalahan saat menghasilkan stiker.');
        await alice.telegram.editMessageText(alice.chat.id, creatingMessage.message_id, null, 'Error!!!');
      }
    } else {
      alice.reply('Silakan balas perintah /sticker pada sebuah foto untuk membuat stiker.');
    }
  });
};